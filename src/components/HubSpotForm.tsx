  ...
  if (submitted) {
    return (
      <div className={`text-center py-10 px-6 ${className}`}>
        <div className="w-16 h-16 bg-[#F5C518] rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg">
          <svg className="w-8 h-8 text-[#3D4466]" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h3 className="text-2xl font-bold text-[#3D4466] mb-3">{thankYouTitle}</h3>
        <p className="text-gray-600 max-w-md mx-auto leading-relaxed mb-6">{thankYouBody}</p>
        {thankYouCta && (
          <Link
            href={thankYouCta.href}
            className="inline-block bg-[#3D4466] text-white font-bold px-6 py-3 rounded-xl hover:bg-[#2a3052] transition-colors"
          >
            {thankYouCta.label}
          </Link>
        )}
